(() => {
  ...
  const input = window.TriadInput = {
    ...
    active: false, x: 0, y: 0, look: 0, fire: false, sprint: false,
    reload: false, fireTap: false, sensitivity: 1,
    reset() {
      pointers.clear();
      this.x = this.y = this.look = 0;
  ...
  function bind(id, kind) {
    ...
    target.addEventListener('pointerdown', e => {
      ...
      pointers.set(e.pointerId, {kind, x: e.clientX, centerX: box.left + box.width / 2, centerY: box.top + box.height / 2});
  ...
  bind('touch-sprint', 'sprint'); bind('touch-reload', 'reload');
