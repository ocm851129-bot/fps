(() => {
'use strict';
const defaults={armor:'standard',magazine:'standard',camo:'woodland'};
const choices={armor:{standard:'표준 / 균형',light:'경량 / HP −15 · 속도 +12%',heavy:'중장갑 / HP +20 · 속도 −12%'},magazine:{standard:'표준 탄창',extended:'확장 / 장탄 +30% · 재장전 +25%',quick:'퀵 체인지 / 장탄 −20% · 재장전 −25%'},camo:{woodland:'우드랜드',urban:'어반 그레이',desert:'데저트 샌드'}};
...
const kits=[0,1,2].map(i=>clean(saved.kits?.[i]));
...
function draw(){drawRank();$('armory-name').textContent=base[selected].name+' / '+base[selected].role;for(const k of Object.keys(choices))$('kit-'+k).value=kits[selected][k];const s=stats(base[selected],kits[selected]);$('kit-stats').textContent=`HP ${s.hp} · 이동 ${s.speed.toFixed(2)} · ${s.mag}발 · 재장전 ${s.reload.toFixed(2)}초`;}
