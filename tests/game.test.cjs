...
test('death waits five seconds and selected gun replaces the next-life loadout',()=>{const {g,$}=setup();g.start();const p=g.player;const enemy=g.units.find(u=>u!==p);p.safe=0;g.damage(p,enemy,999);assert.equal(p.dead,5);$('respawn-gun').onchange({target:{value:'2'}});g.update(.04);assert.equal(p.hp,0);assert.ok(p.dead>4.9);for(let i=0;i<130;i++)g.update(.04);assert.ok(p.hp>0);assert.equal(p.gunId,2);assert.equal(p.ammo,12);assert.equal(p.grenades,2);});
test('knife is range limited and grenades consume finite stock',()=>{const {g}=setup();g.start();const p=g.player,e=g.units.find(u=>u!==p);p.x=2.5;p.y=2.5;p.a=0;p.safe=0;p.cool=0;e.x=3;e.y=2.5;e.safe=0;e.hp=100;p.slot=2;g.shoot(p);assert.equal(e.hp,45);p.cool=0;p.slot=1;g.shoot(p);assert.equal(p.grenades,1);assert.equal(e.hp,45);});
